...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const res = await listTestCases(ticket.id);
        if (cancelled) return;

        setGenerationStatus(res.status);
        if (res.ai_model) setAiModel(res.ai_model);
        if (res.test_cases?.length > 0) {
          setTestCases(res.test_cases);
        }

        if (res.status === "generating" || res.status === "processing") {
          return false;
        }
        return true;
      } catch {
    ...
    <TooltipProvider>
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 pb-28 py-4">
        ...
        <Card className="border-indigo-electric/20 bg-[#1C1C1C]/90 shadow-none">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {aiModel ? (
                    aiModel.toLowerCase().includes("gemini") ? (
                      <Badge className="border-amber-500/40 bg-amber-500/10 text-amber-300 font-mono text-xs">
                        Generated by Gemini (fallback)
                      </Badge>
                    ) : (
                      <Badge className="border-indigo-electric/30 bg-indigo-electric/10 text-indigo-300 font-mono text-xs">
                        Generated by Claude
                      </Badge>
                    )
                  ) : null}
          ...
          <CardContent>
            {generationStatus === "generating" || generationStatus === "processing" ? (
